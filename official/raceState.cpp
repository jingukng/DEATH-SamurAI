...
RaceState::RaceState
(RaceCourse &course,
 string &player0, string &port0, string &player1, string &port1,
 const std::array<Option, 2>& options):
  course(&course),
  players{
    //変更：名前の代わりにポートを使う
  Player(player0, course, course.startX[0], options[0], port0),
    Player(player1, course, course.startX[1], options[1], port1)},
...
(int stepNumber, RaceState &rs) {
  ...
  for (int p = 0; p != 2; p++) {
    ...
    // Initialize Log
    if (ps.state == RACING) {
          playResult[p] =	rs.players[p].plan(stepNumber, rs.players[1-p],
			    *rs.course, rs.visibility,
			    acceleration[p], timeUsed[p]);
    // cout << "accel of p"<<p<<":"<<acceleration[p].x<<","<<acceleration[p].y<<endl;
      switch (playResult[p]) {
        case TIMEDOUT:
        case INVALID:
        case DIED:
