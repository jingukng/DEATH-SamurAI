...
template <class... Args>
void sendToAI(unique_ptr<boost::process::opstream>&  toAI, 
unique_ptr<boost::process::ipstream>& fromAI,
shared_ptr<ofstream> stdinLogStream, const char *fmt, Args... args) {
  ...
  //debug 送信メッセージ確認
  // cout << str;
  //注意：この関数では*fromAI >> bufとかは，やらない方が良い
  //注意：*tiAI << str << endlとはしないこと．flushToAIで送る
...
//---- constructor initiate child process --------//
Player::Player(string command, const RaceCourse &course, int xpos, const Option &opt, string port):
  group(),
  state(PlayerState
  ...
  //変更：ポート受け取ってソケットプログラムに渡す  
  sendToAI(toAI, fromAI, option.stdinLogStream, "%d\n", stoi(port));
  flushToAI(toAI, option.stdinLogStream);

  //変更：ソケットプログラムが返す名前をメンバ変数nameに入れる
  string myname;
  *fromAI >> myname;
  name = myname;
...
plan(int stepNumber, Player &op, RaceCourse &course, int visibility,
  ...
  if (op.state.state == RACING) {
    sendToAI(toAI, fromAI, option.stdinLogStream, "%d %d %d %d\n",
	     op.state.position.x, op.state.position.y,
	     op.state.velocity.x, op.state.velocity.y);
  } else {
    sendToAI(toAI, fromAI, option.stdinLogStream, "0 %d 0 0\n",
	     course.length);
  }
  for (int y = 0; y < course.length; y++) {
    for (int x = 0; x < course.width; ++x) {
      if (x != 0) {
        sendToAI(toAI, fromAI, option.stdinLogStream, " ", 0);
      }
      sendToAI(toAI, fromAI, option.stdinLogStream, "%d",
	       y < visibility ? course.squares[y][x]: -1);
    }
    sendToAI(toAI, fromAI, option.stdinLogStream, "\n", 0);
  }
  flushToAI(toAI, option.stdinLogStream);  
